...
try
{
    ...
    std::vector<size_t> min_path(WIDTH * HEIGHT, std::numeric_limits<size_t>::max());
    ...
    while (!q.empty())
    {
        ...
        for (auto const &op : ops)
        {
            ...
            if (diff(tiles, n.pos, cur.pos, WIDTH) < 2)
            {
                min_path[n.pos.y * WIDTH + n.pos.x] = n.path.size();
    ...
    size_t minp = std::numeric_limits<size_t>::max();
    for (size_t i = 0; i != tiles.size(); ++i)
        if (tiles[i] == 0) // only check elevation 'a'
            minp = std::min(minp, min_path[i]);

    std::cout << "Path length: " << minp << '\n';
}
