...
size_t find_dirs_max_size(Node const &node, size_t max_size)
{
    size_t total = 0;
    ...
    // Also cound all child directories
    for (Node const &c : node.children)
        if (is_dir(c))
            total += find_dirs_max_size(c, max_size);
...
try
{
    ...
    size_t total_size = calculate_dir_size(root);
    std::cout << "total_size: "<< total_size << '\n';

    size_t max_size = find_dirs_max_size(root, 100000);
